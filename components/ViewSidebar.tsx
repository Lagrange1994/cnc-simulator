...
import { Projection, RenderMode, ViewSettings } from '../types';
...
type SceneVisibilityKey = 'machineHousing' | 'fixturesClamps' | 'rapidLines' | 'toolpathHistory';

const ViewSidebar: React.FC<ViewSidebarProps> = ({ onClose, settings, onSettingsChange }) => {
  const toggleSwitch = (key: SceneVisibilityKey) => {
    onSettingsChange({ [key]: !settings[key] });
  };

  const renderModes: { id: RenderMode; label: string; img: string }[] = [
          ...
          <div className="space-y-3">
            {[
              ...
              <div key={item.id} className="flex items-center justify-between p-3 bg-black/20 border border-cds-border/20 chamfer-sm">
                ...
                {/* Carbon Toggle */}
                <button
                  onClick={() => toggleSwitch(item.id as SceneVisibilityKey)}
                  className={`w-10 h-5 relative transition-colors duration-300 ${settings[item.id as SceneVisibilityKey] ? 'bg-cds-interactive/40' : 'bg-cds-layer-03'}`}
                >
                  <div className={`absolute top-0.5 bottom-0.5 w-4 bg-white transition-all duration-300 ${settings[item.id as SceneVisibilityKey] ? 'right-0.5 shadow-[0_0_8px_#4589ff]' : 'left-0.5'}`}></div>
                </button>
              </div>
            ))}
          </div>
        ...
        <section className="p-6 border-t border-cds-border/30 bg-black/10">
          ...
          <div className="grid grid-cols-2 gap-3">
            {renderModes.map((mode) => (
              ...
              >
                {/* Inset chamfer wrapper: the button's own background is the border
                    color, filling the full chamfer-md shape; this wrapper (inset by
                    the p-px padding above) carries the actual image content on a
                    matching, slightly smaller hexagon -- see .chamfer-border in
                    index.css for why a plain `border` can't do this on its own. */}
                <div className="relative w-full h-full chamfer-md-inset overflow-hidden">
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-2">
                    <span className={`text-[10px] font-semibold tracking-widest ${settings.renderMode === mode.id ? 'text-cds-interactive' : 'text-cds-text-01'}`}>{mode.label}</span>
                  </div>
                  {settings.renderMode === mode.id && (
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="flex bg-black/40 p-1 border border-cds-border/20 chamfer-sm">
                {(['PERSPECTIVE', 'ORTHOGRAPHIC'] as Projection[]).map((opt) => (
                  <button
                    key={opt}
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                ...
                <span className="text-[10px] font-mono text-cds-interactive">{settings.gridOpacity}%</span>
              </div>
              <input
                type="range"
                min="0"
                ...
                onChange={(e) => onSettingsChange({ gridOpacity: parseInt(e.target.value, 10) })}
