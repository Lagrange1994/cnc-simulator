...
**What:** Add RPM/feed-rate range validation, fix the two fake gauges in
`Sidebar.tsx` (spindle load hardcodes `fillPercent={status.isSimulating ? 80 : 0}`
at line 41, feed load hardcodes `fillPercent={status.isSimulating ? 40 : 0}` at
line 42 — both ignore the actual RPM/feed values), and add a `Material` type
...
today none of it exists. A full repo-wide search for
hardness/chipload/cutting-force/MRR/chatter/torque returns nothing real. The
app is currently a G-code line-stepper + SVG toolpath visualizer, not a
physics simulator, despite being pitched as one.
...
with the physics logic, not before it. Start by deciding the cutting model
(simplified formula vs. real MRR/chip-load formulas) — that decision drives
the schema shape.

**Effort:** L
**Priority:** P2
**Depends on:** Deciding the cutting physics model (simplified vs. real MRR
formulas) before schema design.
